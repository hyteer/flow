    ...
    node (config.NODE) {
        ...
        stage("构建服务镜像") {
            ...
            for (int i = 0; i < servicesMap.size(); ++i) {
              ...
              build job: 'srv-debug', parameters: [string(name: 'base_dir', value: BASE_DIR), string(name: 'service_name', value: servicesMap[i]['name']), string(name: 'version', value: config.VERSION), string(name: 'run_mode', value: '1')]
            }
